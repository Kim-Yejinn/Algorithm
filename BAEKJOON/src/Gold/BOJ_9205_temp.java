	...
	public static void main(String[] args) {
		...
		for (int tc = 0; tc < t; tc++) {
			...
			s: while (!q.isEmpty()) {
				...
				for (int j = 0; j < n; j++) {
					Node sto = store[j];
					if (visit[sto.x / 50 + offset][sto.y / 50 + offset] == 1) {
						continue;
					}
					if ((Math.abs(sto.x - x) + Math.abs(sto.y - y)) < 50) {
						q.offer(new Node(sto.x, sto.y, 20));
						visit[sto.x / 50 + offset][sto.y / 50 + offset] = 1;
						continue s;
					}
				}
				// 만약 0이라면 sad
				...
				visit[x / 50 + offset][y / 50 + offset] = 1;
				...
				for (int i = 0; i < 4; i++) {
					int nx = x + dx[i];
					int ny = y + dy[i];
					if (visit[nx / 50 + offset][ny / 50 + offset] == 1) {
						continue;
					}
